...
(function (MM) {
    ...
    MM.getStyle = function (el, styleProp) {
        if (el.currentStyle) return el.currentStyle[styleProp]; else if (window.getComputedStyle) return document.defaultView.getComputedStyle(el, null).getPropertyValue(styleProp)
    };
    ...
    MM.Coordinate.prototype = {
        row: 0, column: 0, zoom: 0, toString: function () {
    ...
    MM.Location.distance = function (l1, l2, r) {
        ...
        var deg2rad = Math.PI / 180, a1 = l1.lat * deg2rad, b1 = l1.lon * deg2rad, a2 = l2.lat * deg2rad,
            b2 = l2.lon * deg2rad, c = Math.cos(a1) * Math.cos(b1) * Math.cos(a2) * Math.cos(b2),
            d = Math.cos(a1) * Math.sin(b1) * Math.cos(a2) * Math.sin(b2), e = Math.sin(a1) * Math.sin(a2);
    ...
    MM.Location.interpolate = function (l1, l2, f) {
        ...
        var deg2rad = Math.PI / 180, lat1 = l1.lat * deg2rad, lon1 = l1.lon * deg2rad, lat2 = l2.lat * deg2rad,
            lon2 = l2.lon * deg2rad;
    ...
    MM.Extent = function (north, west, south, east) {
        if (north instanceof MM.Location && west instanceof MM.Location) {
            var northwest = north, southeast = west;
            north = northwest.lat;
            west = northwest.lon;
            south = southeast.lat;
            east = southeast.lon
    ...
    MM.Extent.prototype = {
        north: 0, south: 0, east: 0, west: 0, copy: function () {
            ...
            var len = locations.length, first = locations[0];
    ...
    MM.Transformation.prototype = {
        ax: 0, bx: 0, cx: 0, ay: 0, by: 0, cy: 0, transform: function (point) {
    ...
    MM.Projection.prototype = {
        zoom: 0, transformation: null, rawProject: function (point) {
    ...
    MM.MapProvider.prototype = {
        ...
        sourceCoordinate: function (coord) {
            var TL = this.tileLimits[0].zoomTo(coord.zoom).container(), BR = this.tileLimits[1].zoomTo(coord.zoom),
                columnSize = Math.pow(2, coord.zoom), wrappedColumn;
    ...
    MM.MouseWheelHandler = function () {
        var handler = {id: "MouseWheelHandler"}, map, _zoomDiv, prevTime, precise = false;
    ...
    MM.DoubleClickHandler = function () {
        var handler = {id: "DoubleClickHandler"}, map;
    ...
    MM.DragHandler = function () {
        var handler = {id: "DragHandler"}, prevMouse, map;
    ...
    MM.MouseHandler = function () {
        var handler = {id: "MouseHandler", handlers: []}, map;
        handler.init = function (x) {
            map = x;
    ...
    MM.TouchHandler = function () {
        var handler = {id: "TouchHandler"}, map, maxTapTime = 250, maxTapDistance = 30, maxDoubleTapDelay = 350,
            locations = {}, taps = [], snapToZoom = true, wasPinching = false, lastPinchCenter = null;
        ...
        function touchEnd(e) {
            ...
            for (var i = 0; i < e.changedTouches.length; i += 1) {
                var t = e.changedTouches[i], loc = locations[t.identifier];
                if (!loc || loc.wasPinch) {
                    continue
                }
                var pos = {x: t.clientX, y: t.clientY}, time = now - loc.time,
        ...
        function onDoubleTap(tap) {
            var z = map.getZoom(), tz = Math.round(z) + 1, dz = tz - z;
            var p = new MM.Point(tap.x, tap.y);
            map.zoomByAbout(dz, p)
        }

        function onPanning(touch) {
            var pos = {x: touch.clientX, y: touch.clientY}, prev = locations[touch.identifier];
            map.panBy(pos.x - prev.x, pos.y - prev.y)
        }

        function onPinching(e) {
            var t0 = e.touches[0], t1 = e.touches[1], p0 = new MM.Point(t0.clientX, t0.clientY),
                p1 = new MM.Point(t1.clientX, t1.clientY), l0 = locations[t0.identifier], l1 = locations[t1.identifier];
            l0.wasPinch = true;
            l1.wasPinch = true;
        ...
        function onPinched(p) {
            if (snapToZoom) {
                var z = map.getZoom(), tz = Math.round(z);
    ...
    MM.CallbackManager.prototype = {
        owner: null, callbacks: null, addCallback: function (event, callback) {
            ...
            if (!this._loadComplete) {
                ...
                this._loadComplete = function (e) {
                    ...
                    if (e.type === "load" && (img.complete || img.readyState && img.readyState == "complete")) {
                        ...
                        theManager.dispatchCallback("requesterror", {element: img, url: "" + img.src});
                        img.src = null
                    }
    ...
    MM.Layer.prototype = {
        ...
        inventoryVisibleTile: function (layer_element, tile_coord) {
            var tile_key = tile_coord.toKey(), valid_tile_keys = [tile_key];
    ...
    MM.Map.prototype = {
        ...
        getHandler: function (id, mHandler) {
            var handler = null, rel = this.eventHandlers;
            if (mHandler === true) {
        ...
        removeHandler: function (id) {
            var handler = this.getHandler(id), index = this.eventHandlers.indexOf(handler);
            if (index === -1) {
        ...
        swapLayersAt: function (i, j) {
            ...
            var layer1 = this.layers[i], layer2 = this.layers[j], dummy = document.createElement("div");
    ...
    MM.mapByCenterZoom = function (parent, layerish, location, zoom) {
        var layer = MM.coerceLayer(layerish), map = new MM.Map(parent, layer, false);
